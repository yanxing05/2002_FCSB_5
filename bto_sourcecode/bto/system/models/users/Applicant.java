...
public class Applicant extends User {
    ...
    public boolean canApplyForProject(BTOProject project) {
        if (application != null && !application.getStatus().equals("Unsuccessful") && !application.getStatus().equals("Withdrawn")) {
    ...
    public boolean isEligibleForFlatType(String flatType) {
        if (getMaritalStatus().equals("Married") && getAge() >= 21) {
            return true; // Married applicants >= 21 can apply for any type
        }

        if ("2-Room".equals(flatType)) {
            return getMaritalStatus().equals("Single") && getAge() >= 35;
        }
